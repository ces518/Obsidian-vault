...
IPv4 기반으로 구축된 서버 인프라 환경에서, **엣지(CDN/LB)에서만 IPv6를 수용하고 내부는 IPv4를 유지**하는 현재 업계 주류 패턴과 그 이유를 정리한다.

## 한 줄 요약

> 서버 인프라를 IPv6로 전면 전환하는 것이 아니라, 엣지(CDN/LB)에서 IPv6를 수용하고 중간 홉에서 IPv4로 변환해 내부는 IPv4를 유지하는 것이 현재 실무의 주류 패턴이다.
...
> 서버 환경은 대부분 IPv4 기반이고 VPC도 이미 IPv4로 구축되어 있어서, IPv6를 내부까지 전면 도입하면 보안그룹, ACL, 모니터링, 로깅을 전부 이중 관리해야 하는데 운영 복잡도 대비 실익이 거의 없습니다. 반면 클라이언트 쪽에서는 Apple ATS 정책 때문에 iOS 앱이 IPv6 네트워크에서 동작해야 심사를 통과하고, 모바일 통신사도 IPv4 고갈로 NAT64/DNS64 환경으로 전환하는 추세라 IPv6 대응이 필요합니다. 그래서 실무에서는 CloudFront나 ALB 같은 엣지를 Dual-Stack으로 두어 IPv6 요청을 수신하고, 백엔드에는 IPv4로 전달하는 방식을 씁니다.
