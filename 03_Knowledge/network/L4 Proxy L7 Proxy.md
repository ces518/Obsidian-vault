...
> L4·L7 프록시는 둘 다 User Mode에서 동작하는 **연결 종료형 프록시**라 기반 구조는 같고, 차이는 "받은 스트림으로 무엇을 하느냐"입니다. **L4는 IP·포트만 보고 연결 수립 시 백엔드를 한 번 정한 뒤 바이트를 그대로 중계**해서 빠르고 프로토콜에 무관합니다. **L7은 스트림을 HTTP 같은 프로토콜로 파싱해 요청마다 URL·헤더를 보고 라우팅**하며, 헤더 조작·캐싱·TLS 종료 같은 콘텐츠 개입이 가능한 대신 무겁고 아는 프로토콜만 다룹니다. 그래서 실무에선 **외부는 L4로 빠르게 받고 내부는 L7로 정밀 라우팅하는 2단 구성**을 많이 씁니다. 주의할 점은 IPVS·하드웨어 LB처럼 **연결을 종료하지 않고 패킷 헤더만 포워딩하는 장비는 "L4 LB"라 불려도 프록시가 아니라는 것**입니다. AWS로는 **NLB가 L4, ALB가 L7**에 대응됩니다.
