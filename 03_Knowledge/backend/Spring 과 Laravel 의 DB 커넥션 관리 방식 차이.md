...
Spring(HikariCP)과 Laravel(PHP-FPM)의 DB 커넥션 관리 방식 차이를 정리한 문서.

## 한 줄 요약

> Spring은 애플리케이션 레벨 커넥션 풀로 커넥션을 정책적으로 제한·조절할 수 있는 반면, Laravel(PHP-FPM)은 워커 프로세스 단위 커넥션 재사용에 가까워 그 제어권이 워커/런타임에 있다.
...
> Spring은 HikariCP 같은 애플리케이션 레벨 커넥션 풀을 제공해서 풀 크기, 대기/타임아웃, idle/eviction, 누수 감지 같은 정책으로 DB 커넥션을 의도적으로 제한·조절할 수 있고, 트랜잭션이 끝나면 커넥션이 즉시 풀로 반환됩니다. 반면 Laravel(PHP-FPM)은 풀로 반환되는 개념 없이 워커에 커넥션이 유지된 채 다음 요청에서 재사용되며, 실질적인 상한은 PHP-FPM 워커 수입니다. 그래서 Spring은 동시 커넥션 최대치와 대기·실패 시점을 정책으로 고정해 트래픽 급증 시 백프레셔로 부하를 초기에 누를 수 있지만, Laravel은 워커 재시작/리사이클이 잦아지면 커넥션 재생성이 급증해 DB의 max_connections 상한에 빠르게 근접하면서 DB 자체가 불안정해질 위험이 커집니다.